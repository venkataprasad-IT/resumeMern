...
import { containerStyles, buttonStyles, statusStyles } from "../assets/dummystyle";
import { TitleInput } from "../components/Input";
import { Trash2, Palette, Download, AlertCircle } from "lucide-react";
...
const EditResume = () => {
      ...
      case "profile-info":
        return (
          <ProfileInfoForm
            ...
            onBack={goBack}
            onSaveExit={async () => { await updateResumeDetails(); navigate("/dashboard"); }}
  ...
  const renderPreview = () => {
    const theme = resumeData?.template?.theme || "01";
    const containerWidth = 800;
    let PreviewComponent = TemplateOne;
    if (theme === "02") PreviewComponent = TemplateTwo;
    if (theme === "03") PreviewComponent = TemplateThree;
    return (
      <div className="flex flex-col items-center">
        <div ref={resumeDownloadRef} className="w-full flex justify-center">
          <PreviewComponent resumeData={resumeData} containerWidth={containerWidth} />
        </div>
        <button className="btn-primary mt-6 px-6 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700 transition" onClick={downloadPDF}>Download PDF</button>
      </div>
    );
  };
  ...
  //REMOVING THE INFO USIG THE IND
  ...
  //
  //It wll helpp in choosing the preview as wellas helps in download
  ...
  //UADTE RESUME AND PUT WILL HLP IN UPDATION ON SERVER... SIDE
...
//Downoadng functions.
  ...
  //so all this func used for editing the resye
  //all code 
  return (
    <DashboardLayout>
          ...
          <div className="flex flex-wrap items-center gap-3">
           ...
           <div className={containerStyles.grid}>
              <div className={containerStyles.formContainer}>
                ...
                {renderForm()}
                {currentPage === "preview" && renderPreview()}
      ...
      >
        <ThemeSelector
          ...
          setSelectedTheme={(theme) => updateTheme(theme)}
        ...
        <div className="flex flex-col items-center">
          <div ref={resumeDownloadRef} className="w-full flex justify-center">
            {(() => {
              ...
              if (theme === "02") PreviewComponent = TemplateTwo;
              if (theme === "03") PreviewComponent = TemplateThree;
              return <PreviewComponent resumeData={resumeData} containerWidth={containerWidth} />;
            })()}
          </div>
          <button className="btn-primary mt-6 px-6 py-2 rounded bg-indigo-600 text-white hover:bg-indigo-700 transition" onClick={downloadPDF}>Download PDF</button>
        </div>
